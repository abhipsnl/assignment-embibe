...
MonitorNewEntry() {
    ...
    if [ ${INITIAL_COUNT} -lt ${NEW_COUNT} ];then
        ...
        DisplayMessage "No new entry for - ${PROJ}" "INFO"
    fi
}
      ...
      do
          if [ ! -f "${GIT_USERNAME}-${project}-initial-data.txt" ];then
              ...
              if [ -s "${GIT_USERNAME}-${project}-initial-data.txt" ];then
                  ...
                  curl ${GIT_API}/${GIT_USERNAME}/${project}/contributors 2>>${LOG_FILE} | grep "login" 1>${GIT_USERNAME}-${project}-new-data.txt
                  if [ $? -ne 0 ];then
